...
NeuralNet *nnet_init()
{
    ...
    for (i = 0; i < LAYERS; i++)
    {
        ...
        new_network->weightGradients[i] = laa_allocMatrix(npl[i + 1], npl[i], 0);

        new_network->biases[i] = laa_allocRandVector(npl[i + 1]);
        new_network->biasGradients[i] = laa_allocVector(npl[i + 1], 0);
...
TestingSet *nnet_testing_set_init(int num_testing_examples)
{
    ...
    new_set->testing_examples = num_testing_examples;
    int i;
    new_set->testing_inputs = laa_allocMatrixRaw(num_testing_examples, INPUT_LAYER_SIZE);
    new_set->testing_outputs = laa_allocMatrixRaw(num_testing_examples, OUTPUT_LAYER_SIZE);
    return new_set;
}
...
TrainingSet *nnet_training_set_init(int num_training_examples)
{
    ...
    new_set->training_examples = num_training_examples;
    int i;
    new_set->training_inputs = laa_allocMatrixRaw(num_training_examples, INPUT_LAYER_SIZE);
    new_set->training_outputs = laa_allocMatrixRaw(num_training_examples, OUTPUT_LAYER_SIZE);
    return new_set;
}
...
void nnet_free(NeuralNet *nnet)
{
    ...
    for (i = 0; i < LAYERS; i++)
    {
        ...
        laa_freeMatrix(nnet->weightGradients[i], npl[i + 1]);

        laa_freeVector(nnet->biases[i]);
        laa_freeVector(nnet->biasGradients[i]);
...
{
    laa_freeMatrix(set->testing_inputs, set->testing_examples);
    laa_freeMatrix(set->testing_outputs, set->testing_examples);
    free(set);
}

void nnet_free_training_set(TrainingSet *set)
{
    laa_freeMatrix(set->training_inputs, set->training_examples);
    laa_freeMatrix(set->training_outputs, set->training_examples);
    free(set);
}
...
void nnet_reset_network(NeuralNet *nnet)
{
    ...
    for (i = 0; i < LAYERS; i++)
    {
        ...
        laa_setMatrixTo(nnet->weightGradients[i], npl[i + 1], npl[i], 0);

        laa_setVectorToRand(nnet->biases[i], npl[i + 1]);
        laa_setVectorTo(nnet->biasGradients[i], npl[i + 1], 0);
...
{
    readRowData_ML(fileName, delimiter, bufferSize, training_set->training_examples + testing_set->testing_examples, testing_set->testing_examples, INPUT_LAYER_SIZE, OUTPUT_LAYER_SIZE, training_set->training_inputs, training_set->training_outputs, testing_set->testing_inputs, testing_set->testing_outputs, 1);
    //     laa_printMatrix(training_set->training_inputs, training_set->training_examples, npl[0]);
    //     laa_printMatrix(training_set->training_outputs, training_set->training_examples, npl[LAYERS]);
    //     laa_printMatrix(testing_set->testing_inputs, testing_set->testing_examples, npl[0]);
    //     laa_printMatrix(testing_set->testing_outputs, testing_set->testing_examples, npl[LAYERS]);
}
...
float nnet_total_cost(NeuralNet *nnet, TrainingSet *set)
{
    float sum = 0;
    int i, j;
    for (i = 0; i < set->training_examples; i++)
    {
        nnet_run_data(set->training_inputs[i], nnet);
        for (j = 0; j < npl[LAYERS]; j++)
        {
            sum += (set->training_outputs[i][j] - nnet->activations[LAYERS - 1][j]) * (set->training_outputs[i][j] - nnet->activations[LAYERS - 1][j]) / set->training_examples;
        }
    }
    return sum;
}
    ...
    for (layer = LAYERS - 2; layer > 1; layer--)
    {
        ...
        for (i = 0; i < npl[layer]; i++)
        {
            for (j = 0; j < npl[layer - 1]; j++)
            {
                ...
                nnet->weightGradients[layer - 1][i][j] += dotSum1;
            }
            ...
            nnet->biasGradients[layer - 1][i] += dotSum2;
        }
    }
    multiply_replace(nnet->weight_product, npl[layer + 2], npl[layer + 1], nnet->weights[layer], npl[layer + 1], npl[layer], nnet->weight_product_buffer);
    for (i = 0; i < npl[layer]; i++)
...
{
    const int examples_per_batch = num_mini_batches ? training_set->training_examples / num_mini_batches : 0;
    int iteration;
    int nthExample;
    int layer;
    int batch;
    for (iteration = iterations; iteration--;)
    {
        for (batch = 0; batch < num_mini_batches; batch++)
        {
            ...
            {
                nnet_iterate_gradients(nnet, training_set->training_inputs[nthExample], training_set->training_outputs[nthExample]);
            }
            nnet_subtract_gradients(nnet, learn_rate, training_set->training_examples);
        }
        for (; nthExample < training_set->training_examples; nthExample++)
        {
            nnet_iterate_gradients(nnet, training_set->training_inputs[nthExample], training_set->training_outputs[nthExample]);
        }
        nnet_subtract_gradients(nnet, learn_rate, training_set->training_examples);
...
void update_last_two_layer_gradients(NeuralNet *nnet, float *expected_output)
{
    ...
    for (i = 0; i < npl[LAYERS]; i++)
    {
        ...
        nnet->biasGradients[LAYERS - 1][i] += diff;
        for (j = 0; j < npl[LAYERS - 1]; j++)
        {
            nnet->weightGradients[LAYERS - 1][i][j] += diff * nnet->activations[LAYERS - 2][j];
            ...
            //  the same loops.
            nnet->biasGradients[LAYERS - 2][j] += nnet->weights[LAYERS - 1][i][j] * diff;

            // dot product:
            for (k = 0; k < npl[LAYERS - 2]; k++)
            {
                nnet->weightGradients[LAYERS - 2][j][k] += nnet->weights[LAYERS - 1][i][j] * diff * nnet->activations[LAYERS - 3][k];
            }
        }
    }
}
...
void nnet_subtract_gradients(NeuralNet *nnet, float learn_rate, int num_training_examples)
{
    ...
    for (layer = 0; layer < LAYERS; layer++)
    {
        for (i = 0; i < npl[layer + 1]; i++)
        {
            ...
            {
                nnet->weights[layer][i][j] -= nnet->weightGradients[layer][i][j] * learn_rate / num_training_examples;
                nnet->weightGradients[layer][i][j] = 0;
            }
            nnet->biases[layer][i] -= nnet->biasGradients[layer][i] * learn_rate / num_training_examples;
            nnet->biasGradients[layer][i] = 0;
        }
    }
}
...
{
    int i = 0, numWrong = 0;
    for (i = 0; i < test_set->testing_examples; i++)
    {
        if (laa_maxIndexValue(test_set->testing_outputs[i], npl[LAYERS]) != laa_maxIndexValue(nnet_run_data(test_set->testing_inputs[i], nnet), npl[LAYERS]))
            numWrong++;

        if (print_each_test)
        {
            printf("\ndesired output: ");
            laa_printVector(test_set->testing_outputs[i], npl[LAYERS]);
    ...
    if (print_results)
    {
        printf("\naccuracy: %d/%d (%.3f%%)\n", test_set->testing_examples - numWrong, test_set->testing_examples, 100 * (test_set->testing_examples - numWrong) / (float)test_set->testing_examples);
    }
    return (test_set->testing_examples - numWrong) / (float)test_set->testing_examples;
}
